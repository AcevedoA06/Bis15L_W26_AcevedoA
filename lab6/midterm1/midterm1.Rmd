...
https://github.com/AcevedoA06/Bis15L_W26_AcevedoA
...
```{r}
anolis_clean <- clean_names(anolis)
anolis_clean
```
...
anolis_cvrt %>% 
 distinct(island)
#The islands represented in the data are Cuba, Hispaniola, Puerto Rico, and Jamaica
```
...
anolis_cvrt %>% 
  ...
  geom_bar()+
  labs(title="Number of Anoles on Each Island Represented in the Data")
# The sample is not even as the majority of anoles come from Cuba or Hispaniola
```
...
anolis_cvrt %>% 
  distinct(habitat)
#There are 4 habitat types in the data which are Trunk-ground, Trunk-crown, living in twigs, or living in the grass-brush 
```
...
anolis_cvrt %>% 
  ...
  geom_bar()+
  labs(title="Number of Anoles of Each Habitat Type Represented in the Data")
#Although more evenly dispersed than the island, a noticeable majority of anoles in the data live in the ground level of tree trunks or in grass-bush
```
...
anolis_cvrt %>% 
  ...
  geom_boxplot()+
  labs(title="Hindlimb Length in cm vs habitat type of anoles")
#Their is a distinction among the anoles where those that live in trees tend to have longer limbs than the ones that live in other forms of vegetation
```
...
```{r}
 anolis_final <- anolis_cvrt %>% 
  mutate(ratio_hindlimb_to_body=hindlimb_length_mm / body_length_mm,
...
```{r}
anolis_final %>% 
  ggplot(mapping=aes(x=habitat, y=ratio_hindlimb_to_body, fill=habitat))+
  geom_boxplot()+
  labs(title="Hindlimb to Body Ratio vs habitat type of anoles")
#This plot has tree crowns, ground trunks and twigs all maintain their postition relative to each other, grass-bush shot up showing that despite not having the tallest hindlimbs by raw lenght, they have the 2nd most amount of limb to bodysize
```
...
```{r}
anolis_final %>% 
  ggplot(mapping=aes(y=ratio_of_tail_to_body, x=ratio_hindlimb_to_body, color=habitat))+
  geom_point()+
  geom_smooth(method=lm)
  labs(title="Hindlimb to Body Ratio vs Tail to Body Ratio, Shown by species")
  #This plot suggest that an anole who has a short limb to body ratio will most likely have a short tail to body ratio. Grass-bush lizards have the highest overall appendage to body ratio, which in interesting since the rest of the data shows that compared to the rest of the anoles they have an average raw appendage length. 
```
...
anolis_final %>% 
  ...
  summarize(mean_toepad_lamellae_count_grass_bush=mean(toepad_lamellae_count, na.rm=T))
```

```{r}
anolis_final %>% 
  filter(habitat=="Trunk-crown") %>% 
  summarize(mean_toepad_lamellae_trunk_crown=mean(toepad_lamellae_count, na.rm=T))
```
```{r}
anolis_final %>% 
  filter(habitat=="Trunk-ground") %>% 
  summarize(mean_toepad_lamellae_trunk_ground=mean(toepad_lamellae_count, na.rm=T))
```
...
anolis_final %>% 
  filter(habitat=="Twig") %>% 
  summarize(mean_toepad_lamellae_twig=mean(toepad_lamellae_count, na.rm=T))
```
...
anolis_final %>% 
  filter(habitat=="Trunk-crown") %>% 
  ggplot(mapping=aes(y=toepad_lamellae_count,x=island, fill=island))+
  geom_boxplot()+
  labs(title="Toepad Lamellae Count vs Island for Trunk-Crown Species")
```
