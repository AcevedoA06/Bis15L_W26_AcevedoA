...
```{r}
penguins %>% 
  filter(island==c("Biscoe", "Dream")) #3 diff ways to do misma shit
```
...
```{r}
penguins %>% 
  distinct(species, island, .keep_all=T) #Keep_all is to keep all other variables of what distinct wanted
```
...
```{r}
mammals <- read_csv("data/mammal_lifehistories_v2.csv") %>% clean_names #attach clean names when loading in data to get rid of bs in names from the start
```
...
```{r}
mammals %>% 
  mutate(across(everything(), tolower)) #Carefiul w ts cause it turns numerics into characters 
```

For this reason, it might be better to use `where` so we can specify only character columns.
```{r}
mammals %>% 
  mutate(across(where(is.character), tolower)) #This will mess only w columns that are already characters
```
...
mammals %>% 
  ...
  select(gestation_new, newborn_new) %>% 
  ggplot(mapping = aes(x=gestation_new, y=newborn_new)) +
  geom_point()+
