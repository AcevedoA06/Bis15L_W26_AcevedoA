...
```{r}
mt_name <-c("everest", "k2", "kangchenjunga", "lhotse", "makalu") 
mt_ft_height <- c(29032, 28251, 28169, 27940, 27838)
mt_m_height <- c(8849, 8611, 8586, 8516, 8485)
```

**2. Combine these vectors into a data frame called `mountains`.**
```{r}
mtd <- tibble(mt_name, mt_ft_height, mt_m_height,) #mtd short for mountain data
```


**3. What is the mean height of the mountains in feet?**
```{r}
mean(mtd$mt_ft_height)
```
...
mtd_withfclimbed <- tibble(mtd , first_climbed)
```
...
```{r}
summits <- c(12884, 800, 500, 950, 450)
mtd_fclimb_summits <- tibble(mtd_withfclimbed, summits)
mtd_fclimb_summits
```
...
```{r}
fatalities <- c(340, 96, 52, 32, 50)
mtd_fclimb_summits_fatality <-tibble(mtd_fclimb_summits, fatalities) 
mtd_fclimb_summits_fatality
```
...
```{r}
fatality_rate <- c(2, 25, 22, 3, 11.5)
mtd_definitive <- tibble(mtd_fclimb_summits_fatality, fatality_rate)
mtd_definitive
```

**8. Write your data frame to a .csv file called `mountains_data`.**
```{r}
write.csv(mtd_definitive, "mountains_data", row.names=FALSE)
```
...
```{r}
summary(mountains) #not sure if asking for summary() function specifically or any summary function 
glimpse(mountains)
dim(mountains)
```
