...
```{r}
left_join(table_A, table_B, by="customer_ID") #keeps full left table and only matching of right
```
...
```{r}
right_join(table_A, table_B, by="customer_ID")  #keeps full right table and only matching of right
```
...
```{r}
anti_join(table_A, table_B, by="customer_ID") #use to troubleshoot problematic joints
```
...
```{r message=FALSE, warning=FALSE}
spider_r <- read_csv("data/spider_records.csv") %>% clean_names()
```

```{r, message = FALSE}
spider_l <- read_csv("data/spiders_locations.csv") %>% clean_names()
```

2. Join the files using an appropriate key and join type.
```{r}
spiders_all<- left_join(spider_r, spider_l, by="accession")
```
...
```{r}
spiders_all <- spiders_all %>% 
  mutate(date=dmy(date))
```

4. Save your final data to a `.csv`.
```{r}
write.csv(spiders_all, file="spiders_with_locs.csv")
```
