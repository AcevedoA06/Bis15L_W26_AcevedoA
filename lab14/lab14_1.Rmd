...
spiders <- read_csv("data/spiders_with_locs.csv")%>% 
  clean_names() %>% 
  filter(latitude<=42) #filters data that says spider lives in ocean (don't)
```
...
lat <- c(34.67, 41.80)
long <- c(-124.1,-115.5)
bbox <- make_bbox(long, lat, f=0.03) #f= creates buffer
```

Let's get a base map for our bounding box area. There are several different map [styles](https://docs.stadiamaps.com/themes/)
```{r}
map1 <- get_stadiamap(bbox, maptype = "stamen_terrain", zoom=7) #zoom used to create more or less m deatils
```
...
leaflet(spiders) %>% 
  ...
  addCircleMarkers(
    ...
    lat = ~latitude,
    radius = 2,
    stroke = FALSE, #removes lil box around circles
    fillOpacity = 0.7
  ) %>% 
...
griz_lon <- c(-131.3, -166.2)
```


3. Load a map from `stadiamaps` in a `stamen_terrain` projection and display the map.
```{r}
griz_map1 <- get_stadiamap(bbox, maptype = "stamen_terrain", zoom=7)
```
...
```{r}
ggmap(griz_map1)+
  geom_point(data=grizzly, aes(latitude, longitude), size=0.8)+
  labs(x="Longitude", y="latitude", title = "Urosus Horriblus")
```
