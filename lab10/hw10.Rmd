...
#Santa Barbara and San Diego counties are the 2 biggest hotspots for shark attack incidents
```
...
sharks %>% 
  ...
  summarise(total_incidents=sum(incident_num, na.rm=T)) %>% 
  slice_max(total_incidents, n=5) %>% 
  ggplot(mapping=aes(y=total_incidents, x=county, fill=county))+
  geom_col()
```
...
sharks %>% 
  group_by(month) %>% 
  summarize(incidents_by_month=sum(incident_num, na.rm=T))
```
...
sharks %>% 
  group_by(month) %>% 
  summarize(incidents_by_month=sum(incident_num, na.rm=T)) %>% 
  ggplot(aes(y=incidents_by_month, x=month, fill=month))+
  geom_col()
```
