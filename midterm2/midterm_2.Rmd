...
```{r}
tibble(polar_bears)
#If you go through the tibble of the polar_bears data, NA is represented by the phrase "unkown" for both mating and denning. No other variables appear to have NA's
```
...
polar_bears_na <- polar_bears %>% 
                   replace_with_na(replace =  list(mating="unknown",
                  denning="unknown"))
miss_var_summary(polar_bears_na)
```
...
polar_bears_na %>% 
  ...
  geom_bar()+
  labs(title = "Number of Female vs Male Bears in the Study",
       x="Sex",
       y=NULL)
#Sampling is not perfectly even for both sexes, as their are about 70 more females than males in the study
```
...
polar_bears %>% 
  ...
  labs(title = "Number of Bears Sampled from Each Population",
       y=NULL,
       x="Year")
...
#Min urea_nitrogen is 1, the median is 11, and the max urea_nitrogen content is 69
```

```{r}
polar_bears_na %>% 
  summarize(min_creatinine=min(creatinine),
            median_creatinine=median(creatinine),
            max_creatinine=max(creatinine))
#Min creatinine is 0.35, the median is 1.05, and the max creatinine content is 2.35
```
...
```{r}
polar_bears_na_uncr <- polar_bears_na %>%
                        mutate(un_creat_ratio=(urea_nitrogen*2)/creatinine)
polar_bears_na_uncr
```
...
```{r}
polar_bears_na_uncr_fast <- polar_bears_na_uncr %>% 
                            mutate(fasting_status=case_when(un_creat_ratio<=10 ~ "fasting",
                                            un_creat_ratio>=10 ~ "not_fasting"))
polar_bears_na_uncr_fast
```
...
polar_bears_na_uncr_fast %>% 
  separate(capture_date, into = c("year", "month", "day"), sep= "-") %>% 
  filter(fasting_status=="fasting") %>% 
   ggplot(aes(x=year, fill=popn)) +
   geom_bar(position = "dodge")+
   theme(axis.text.x = element_text(angle = 60, hjust = 1))+
  labs(title = "Number of Fasting Bears in Each Population",
       x="Year",
       y=NULL)
```
...
```{r}
polar_map <- get_stadiamap(bbox, maptype = "stamen_terrain", zoom=7)
ggmap(polar_map)
```
```{r}
ggmap(polar_map) +
  ...
  labs(x="Longitude",
       y="Latitude",
       title = "Polar Bear Locations Used in Study")
```
...
server <- function(input, output, session) {
  ...
  output$plot <- renderPlot({
    ...
    polar_bears_na_uncr_fast %>% 
      ...
      ggplot(aes(y = un_creat_ratio,
                 x = .data[[input$y]]
                 ))+
      geom_boxplot()+
      labs(x="Un_Creat_Ratio",
           fill="Fill Type")+
      theme_minimal()
  })
}
