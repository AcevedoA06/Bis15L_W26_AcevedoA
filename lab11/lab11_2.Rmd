...
## Practice
1. Build a scatterplot that compares `log10_mass` and `log10_preymass`. Color by `thermoreguation` Store this plot as object `q`.
```{r}
q <- homerange %>% 
...
```{r}
my_palette <- paletteer_d("palettetown::zapdos")
my_palette
```
...
homerange %>% 
  ...
  geom_boxplot()+
  facet_wrap(~taxon, nrow=2, labeller=label_both)
  theme(axis.text.x = element_text(angle = 60, hjust=1))+
  theme(axis.text.x  = element_text(angle=60, hjust=1),
...
homerange %>% 
  ggplot(aes(x=taxon, y=log10_mass, fill=thermoregulation))+
  geom_boxplot()+
  facet_grid(~taxon, labeller=label_both)+
  theme(axis.text.x = element_text(angle = 60, hjust=1))+
  theme(axis.text.x  = element_text(angle=60, hjust=1),
        legend.position = "none",
        strip.text=element_text(size=12, face="bold"))
