...
```{r}
 gap %>% 
  ...
  labs(title = "GDP per Capitia in the USA, 2000-2025",
       x="Year",
       y="GDP")
```
...
gap %>% 
  filter(year==1935) %>% 
  ggplot(aes(x=lex))+
  geom_histogram(fill="turquoise4", color="black", bins = 30)+
  labs(title = "Distributionof life expentancy")
```
...
gap %>% 
  filter(year==1935) %>% 
  ggplot(aes(x=lex))+
  geom_density(fill="turquoise4", color="black", alpha=0.7)+
  labs(title = "Distributionof life expentancy")
```
...
gap %>%
  filter(year==1935) %>%
  ggplot(aes(x=lex))+
  geom_histogram(aes(y=after_stat(density)), color="black", fill="turquoise4", alpha=0.4)+
